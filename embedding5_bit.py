...
def clean_output(text):
    # Remove repetitive question marks and other punctuation
...
def get_user_preferences(review_texts, summaries):
    """Generate user preferences in batch using Llama 2 Chat based on review texts and summaries."""
    print("Get User Preferences started...")
    system_prompt = "<s> [INST] <<SYS>> You are a helpful, respectful and honest assistant. Always answer as helpfully as possible, while being safe. Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive in nature. If a question does not make any sense, or is not factually coherent, explain why instead of answering something not correct. If you don’t know the answer to a question, please don’t share false information.<</SYS>>"

    # Create batch prompts
    prompts = [
        f"""{system_prompt} The user left the following review for a product:\n\n'{text}'.\n\nThe product summary is:\n\n'{sum}'.\n\nBased on the review and summary of a product, can you analyze the user's preferences (be as specific as you can)? [\INST]"""
        for text, sum in zip(review_texts, summaries)
    ]

    input_ids = base_tokenizer(prompts, return_tensors="pt", padding=True, truncation=True).input_ids.cuda()
    outputs = base_model.generate(input_ids=input_ids, max_new_tokens=4096)
    outputs_decoded = base_tokenizer.batch_decode(outputs.detach().cpu().numpy(), skip_special_tokens=True)
    
    # Extract preferences from the prompts (potentially slicing off system prompts)
    #return [out[len(prompt):].strip() for out, prompt in zip(outputs_decoded, prompts)]
    # Extract preferences from the prompts (potentially slicing off system prompts)
    preferences = [out[len(prompt):].strip() for out, prompt in zip(outputs_decoded, prompts)]

    # Print the extracted preferences
    print("Extracted User Preferences:")
    for preference in preferences:
        print(preference)
    
    # Apply cleaning to each preference extracted
    preferences = [clean_output(pref) for pref in preferences]
    
    # Print the extracted preferences
    print("Extracted User Preferences After Processing ????????:")
    for preference in preferences:
        print(preference)
...
def process_dataset(df, tokenizer, bert_model):
    ...
    df_sorted = df.sort_values(by='reviewerID').head(6)
    print("Sorting done")
    ...
    # Assume batch_size is set appropriately for your GPU
    batch_size = 2

    for start in range(0, len(df_sorted), batch_size):
        end = start + batch_size
        batch = df_sorted.iloc[start:end]
        batch_preferences = get_user_preferences(batch['reviewText'].tolist(), batch['summary'].tolist())
        user_preferences.extend(batch_preferences)
        batch_embeddings = get_bert_embedding(batch_preferences)
